(() => {
 ...
 const first=parts();if(legacyRedirect(first))return;
 if(floors.some(f=>f.id===first[0]))browseFloor=first[0];
 search.value=new URLSearchParams(location.search).get('q')||'';
 document.querySelector('#room-floors').innerHTML='<a href="#/all">All rooms</a>'+floors.map(f=>`<a href="#/${f.id}">${esc(f.name)}</a>`).join('');
 function renderGrid(){const q=search.value.toLocaleLowerCase().trim(),key=browseFloor+'|'+q;if(key===gridKey)return;gridKey=key;visible=groups.filter(g=>(browseFloor==='all'||g.floor.id===browseFloor)&&[g.name,g.floor.name,...g.views.map(v=>v.title+' '+v.description)].join(' ').toLocaleLowerCase().includes(q));
 grid.innerHTML=visible.length?visible.map(g=>`<a class="room-card" href="${url(g)}"><div class="room-card-image"><img src="${esc(g.views[0].path)}" alt="${esc(g.name)}" width="1536" height="1024" loading="lazy"><span class="view-badge">${g.views.length} ${g.views.length===1?'view':'views'}</span></div><h2>${esc(g.name)}</h2><p>${esc(g.floor.name)}</p></a>`).join(''):'<div class="room-empty"><h2>No matching rooms</h2><p>Try another name or floor.</p><button type="button" id="room-clear">Show all rooms</button></div>';
 ...
 function render(){const p=parts();if(legacyRedirect(p))return;selected=null;const f=floors.find(f=>f.id===p[0]);
 ...
 document.querySelector('#room-position').textContent=g.floor.name+' · '+g.views.length+(g.views.length===1?' view':' views');document.querySelector('#room-title').textContent=g.name;
 ...
 document.querySelector('#plan-floor').innerHTML=floors.map(f=>`<option value="${f.id}">${esc(f.name)}</option>`).join('');
 function renderPlan(){const f=floors.find(f=>f.id===document.querySelector('#plan-floor').value);document.querySelector('#plan-image').src=f.plan;document.querySelector('#plan-image').alt=f.name+' plan of Albury';document.querySelector('#plan-original').href=f.plan;document.querySelector('#plan-title').textContent=f.name+' · plan'}
